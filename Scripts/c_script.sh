...
for i in *
{
	...
	if [[ -f $j ]]; then
 			...
 			if [[ $i == "thread-ring" ]] || [[ $i == "fannkuch-redux" ]]; then
 				 /usr/bin/time -f "%e\t%M " -o output /usr/bin/gcc -pipe -Wall -O3 -fomit-frame-pointer -march=native -pthread -falign-labels=8 $j -o a.out
			elif [[ $i == "pidigits" ]]; then
			 	 /usr/bin/time -f "%e\t%M " -o output /usr/bin/gcc -pipe -Wall -O3 -fomit-frame-pointer -march=native $j -o a.out -lgmp 			
 			else	
